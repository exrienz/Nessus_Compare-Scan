...
for report in *.csv; do
  ...
  rm -f -- "${report}.tmp"
  # Remove all TXT files except the specified ones
  find . -type f -name '*.txt' ! -name 'old-vulns.hosts.with.ports.txt' ! -name 'new-vulns.hosts.with.ports.txt' -exec rm {} \;
done


## Filter
sed '/None/d' old-vulns.hosts.with.ports.txt > old.txt
sed '/None/d' new-vulns.hosts.with.ports.txt > new.txt

find . -type f -name '*.txt' ! -name 'old.txt' ! -name 'new.txt' -exec rm {} \;

python3 diff.py > report.csv
